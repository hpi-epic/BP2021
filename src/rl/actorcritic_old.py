...
class ActorCriticAgent(vendors.Agent):
	...
	def train_batch(self, states, actions, rewards, states_dash):
		...
		with torch.no_grad():
			constant = (v_expected - v_estimat).detach()
...
total_envs = 128
environments = [sim_market.CircularEconomyRebuyPriceOneCompetitor() for _ in range(total_envs)]
