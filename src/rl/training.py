...
def train_QLearning_agent(agent, marketplace, maxsteps=2 * config.EPSILON_DECAY_LAST_FRAME, log_dir_prepend='') -> None:
	"""
	Train a QLearningAgent on a market environment.

	Args:
		agent (agents.vendors instance): The agent that should be trained.
		marketplace (market.sim_market instance): The market environment used for the training.
	...
	"""
	assert isinstance(agent, vendors.QLearningAgent), f'the passed agent must be a QLearningAgent: {agent}'
	...
	signature = f'{type(marketplace).__name__}_{type(agent).__name__}'
	...
	for frame_idx in range(maxsteps):
		...
		if len(agent.buffer) < config.REPLAY_START_SIZE:
			continue
	...
	if best_mean_reward == 0:
		...
		print(os.path.abspath(os.path.join('results', 'trainedModels', f'{type(marketplace).__name__}_{type(agent).__name__}_{curr_time}')))
