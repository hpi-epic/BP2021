...
for market_class in market_classes:
    ...
    for i in range(8):
        ...
        if issubclass(market_class,
                      (circular_market.CircularEconomyMonopoly, circular_market.CircularEconomyRebuyPriceMonopoly)) \
                and (i // 4) % 2:
            continue
...
market_initialization_and_steps_testcases = [(*market_related, *agent_related)
                                             for market_related, agent_related in
                                             ut.cartesian_product(market_combinations, agent_classes)]
market_initialization_and_steps_testcases = list(
    filter(lambda x: not (issubclass(x[2], (StableBaselinesA2C, StableBaselinesPPO)) and
...
def test_market_initialization_and_steps(market_class, config_market, agent_class, config_agent,
                                         continuos_action_space):
