...
# 	mock_json = json.dumps(ut_t.create_hyperparameter_mock_dict(sim_market=HyperparameterConfigLoader.load('market_config')))
...
# missing_max_storage = (ut_t.remove_key('max_storage', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing max_storage')
# missing_episode_length = (ut_t.remove_key('episode_length', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing episode_length')
# missing_max_price = (ut_t.remove_key('max_price', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing max_price')
# missing_max_quality = (ut_t.remove_key('max_quality', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing max_quality')
# missing_number_of_customers = (ut_t.remove_key('number_of_customers', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing number_of_customers')
# missing_production_price = (ut_t.remove_key('production_price', HyperparameterConfigLoader.load('market_config')),
# 	'your config is missing production_price')
# missing_storage_cost = (ut_t.remove_key('storage_cost_per_product', HyperparameterConfigLoader.load('market_config')),
...
# @pytest.mark.parametrize('config_market, expected_message', invalid_values_testcases)
# def test_invalid_values(config_market, expected_message):
# 	mock_json = json.dumps(config_market)
# 	with patch('builtins.open', mock_open(read_data=mock_json)) as mock_file:
# 		ut_t.check_mock_file(mock_file, mock_json)
