...
def test_rule_based_circular_competitor2():
    pa = PolicyAnalyzer(RuleBasedCERebuyAgent())
    pa.analyze_policy(np.array([-1, -1]), [(1, "self storage stock", range(0, 30)), (0, "in circulation", range(0, 200))], "refurbished price", 0)
...
def test_circular_monopoly_q_learning(title, policyaccess, expected_filename):
    q_learing_agent = QLearningCERebuyAgent(2, 1000, load_path=os.path.join(parameters_path, 'CircularEconomyRebuyPriceMonopolyScenario_QLearningCERebuyAgent.dat'))
    pa = PolicyAnalyzer(q_learing_agent)
    pa.analyze_policy(np.array([-1, -1]), [(1, "self storage stock", range(0, 30)), (0, "in circulation", range(0, 200))], title, policyaccess)
...
def test_circular_duopol_continuos_actorcritic(title, policyaccess, expected_filename):
    q_learing_agent = ContinuosActorCriticAgentFixedOneStd(6, 3, load_path=os.path.join(parameters_path, 'actor_parametersCircularEconomyRebuyPriceOneCompetitor_ContinuosActorCriticAgentFixedOneStd.dat'))
    pa = PolicyAnalyzer(q_learing_agent)
    pa.analyze_policy(np.array([75, 10, -1, -1, 2, 12]), [(1, "competitor's refurbished price", range(0, 10)), (0, "competitor's new price", range(0, 10))], title, policyaccess)
    assert os.path.exists(os.path.join(write_to_path, expected_filename))