...
model = nn.Sequential(
    nn.Linear(4, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU(), nn.Linear(128, 3)
).to('cpu')
model.load_state_dict(
    torch.load('best_marketplace.dat', map_location=torch.device('cpu'))
)

env = SimMarket()  # lksdfg
our_profit = 0
is_done = False
...
print(
    ...
    + str(round(our_profit / state[1], 3))
    + ' and his competitor '
    + str(env.comp_profit)
    + ' with a profit/quality of: '
    + str(round(env.comp_profit / state[3], 3))
)
print(
    'The value of the net is estimated as: '
    + str(our_profit / (state[3] - state[1] + 100))
)
