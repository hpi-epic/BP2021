...
class ConfigParserTest(TestCase):
	...
	def test_complete_config(self):
		# a standard config when the user did not input anything into the form
		test_config = {
			'environment': {
				'task': 'training',
				'marketplace': 'recommerce.market.circular.circular_sim_market.CircularEconomyDuopoly',
				'agents': [
					{
						'name': '',
						'agent_class': 'recommerce.rl.actorcritic.actorcritic_agent.ContinuousActorCriticAgentEstimatingStd',
						'argument': ''
					},
					{
						'name': '',
						'agent_class': 'recommerce.market.circular.circular_vendors.FixedPriceCEAgent',
						'argument': ''
					}
				],
				'enable_live_draw': False
		...
		actual = self.flat_parser._complete_config(test_config)
		assert expected_config == actual, f'expected:\n{expected_config}\n\nGot:\n{actual}'
