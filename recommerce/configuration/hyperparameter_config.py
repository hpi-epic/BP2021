...
class HyperparameterConfigValidator():
    ...
    def _validate_keys(cls, config: dict, demanded_fields: list) -> dict:
        ...
        demanded_keys = {field for field, _ in demanded_fields}
        ...
        if config_keys != demanded_keys:
            ...
            if missing_keys:
                for key in missing_keys:
                    # if a boolean value is missing from the config, we assume the user wants its value to be `False`
    ...
    def _check_types(cls, config: dict, configurable_fields: list, must_contain: bool = True) -> None:
        for field_name, type, _ in configurable_fields:
            try:
                assert isinstance(config[field_name],
                                  type), f'{field_name} must be a {type} but was {type(config[field_name])}'
...
class HyperparameterConfigLoader():
    ...
    @classmethod
    def remove_unrequired_args(cls, hyperparameter_config):
        if 'config_type' in hyperparameter_config and hyperparameter_config['config_type'] == 'rl':
            del hyperparameter_config['config_type']
    ...
    def load(cls, filename: str, checked_class: SimMarket or Agent) -> AttrDict:
        """
        Load the market configuration json file from the `configuration_files` folder, validate all keys and return
        an AttrDict instance. This can only be done after the relevant `environment_config` has been loaded,
        if both are needed, as the checked_class needs to be known.
        ...
        assert issubclass(checked_class,
                          JSONConfigurable), \
            f'the provided checked_class must be a subclass of JSONConfigurable: {checked_class}'
        ...
        hyperparameter_config = cls.remove_unrequired_args(hyperparameter_config)
