...
def run_training_session(marketplace=circular_market.CircularEconomy(competitors=[RuleBasedCERebuyAgentCompetitive()]), agent=q_learning_agent.QLearningCERebuyAgent):
	"""
	...
	Args:
		...
		Defaults to circular_market.CircularEconomy(competitors=[RuleBasedCERebuyAgentCompetitive()]).
	...
	"""
	assert isinstance(marketplace, sim_market.SimMarket), \
	...
	assert issubclass(agent, CircularAgent) == isinstance(marketplace, circular_market.CircularEconomy), \
...
def train_q_learning_classic_scenario():
	...
	"""
	run_training_session(linear_market.LinearEconomy(competitors=[CompetitorLinearRatio1]), q_learning_agent.QLearningLEAgent)
...
def train_q_learning_circular_economy_rebuy():
	...
	"""
	run_training_session(circular_market.CircularEconomy(competitors=[RuleBasedCERebuyAgentCompetitive()]), q_learning_agent.QLearningCERebuyAgent)
...
def train_continuos_a2c_circular_economy_rebuy():
	...
	"""
	run_training_session(circular_market.CircularEconomy(competitors=[RuleBasedCERebuyAgentCompetitive()]), actorcritic_agent.ContinuosActorCriticAgentFixedOneStd)
