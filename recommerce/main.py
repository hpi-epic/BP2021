...
def handle_get_defaults() -> None:  # pragma: no cover only library calls
    ...
    """
    shutil.copytree(os.path.join(os.path.dirname(__file__), 'default_data'),
                    os.path.join(path_manager.PathManager.user_path, 'default_data'),
                    dirs_exist_ok=True)
...
locations to be used by `recommerce`. WARNING: Any existing files with the same name as the default files will be overwritten!""")
    ...
    parser.add_argument('-v', '--version', action='version',
                        version=f'Recommerce Version {metadata.version("recommerce")}')
