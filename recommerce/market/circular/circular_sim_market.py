...
class CircularEconomy(SimMarket, ABC):
    ...
    def get_competitor_classes() -> list:
        ...
        return sorted(ut.filtered_class_str_from_dir(
            'recommerce.market.circular.circular_vendors',
            dir(c_vendors),
            '.*CE(?!Rebuy).*Agent.*'))
    ...
    def _is_probability_distribution_fitting_exactly(self, probability_distribution) -> bool:
        """
        The probability distribution must have one entry for buy_noting and two entries (purchases_new, purchases_refurbished) for every vendor. # noqa: E501
        Args:
    ...
    def _get_competitor_list(self) -> list:
        return [
            ...
            circular_vendors.FixedPriceCERebuyAgent(
                config_market=self.config, fixed_price=(3, 6, 2)),
