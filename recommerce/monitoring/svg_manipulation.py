...
class SVGManipulator():
    def __init__(self, save_dir: str = 'svg', is_linear=False) -> None:
        self.value_dictionary = get_default_dict()
        # do not change the values in template_svg
        with open(os.path.join(os.path.dirname(__file__), 'data',
                  'MarketOverview_template_linear.svg' if is_linear else 'MarketOverview_template.svg'), 'r') as template_svg:
    ...
    def construct_slideshow_html(self, images: list, time: int = 1000) -> str:
        ...
        """
        # slideshow from: https://stackoverflow.com/questions/52478683/display-a-sequence-of-images-in-1-position-stored-in-an-object-js
        return '<!doctype html>\n' + \
            '<html lang="de">\n' + \
