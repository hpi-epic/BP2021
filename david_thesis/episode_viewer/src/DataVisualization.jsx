...
export const DataVisualization = ({rawData}) => {
    ...
    React.useEffect(() => {
        ...
        setPriceGraph(getPriceData(sliderValue, episodeLength, rawData, activeVendor));
        setCustBehaviourGraph(getCustomerBehaviour(sliderValue, episodeLength, rawData, activeVendor));
        setProfitGraph(getProfitGraph(sliderValue, episodeLength, data, activeVendor));
        ...
        <Box sx={{display: 'flex', height: "100%", width: "100%", flexDirection: "column"}}>
            <Box sx={{display: 'flex', justifyContent: "space-between", alignItems: "center"}}>
                <Box className={"statistics_boxes"}>
                    <Card>
                        <CardContent>
                            <Typography variant="h5"># vendors: {rawData["vendors"]}</Typography>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent>
                            <Typography variant="h5">episode length: {episodeLength}</Typography>
                        </CardContent>
                    </Card>
                    <p></p>
                </Box>
            ...
            {rawData && rawData["vendors"] > 1 &&
                <FormControl sx={{display: "flex", flexDirection: 'row', alignItems: "center", marginTop: "1rem"}}>
                    ...
                    > {Array.from(Array(rawData["vendors"]).keys()).map(i => {
                        return <FormControlLabel value={i} control={<Radio/>} label={`Vendor ${i}`}/>
                    })}
            ...
            <Box sx={{display: "flex", marginTop: "2rem"}}>
                {priceGraph && <Box sx={{flexGrow: "1"}}><Line options={{
                    responsive: true,
                    plugins: {
                        legend: {
                            position: 'top',
                ...
                {custBehaviourGraph && <Box sx={{height: "100%", width: "200px", display: "flex"}}><Bar options={{
                    responsive: true,
                ...
                }} data={custBehaviourGraph}/></Box>}
            </Box>

            <Box sx={{marginTop: "3rem"}} className={"statistics_boxes"}>
                <Card>
                    <CardContent>
                        <Typography variant="h5"><strong>in
                            circulation</strong>: {rawData["in_circulation"][sliderValue]}</Typography>
                    </CardContent>
                </Card>
                {activeVendor !== -1 && <Card>
                    <CardContent>
                        <Typography variant="h5">
                            <strong>storage
                                vendor {activeVendor}</strong>: {rawData["in_storage"][activeVendor][sliderValue]} / {rawData["max_storage"]}
                        </Typography>
                    </CardContent>
                </Card>}
                {activeVendor === -1 && Array.from(Array(rawData["vendors"]).keys()).map(i => {
                    return <>
                        <Card>
                            <CardContent>
                                <Typography variant="h5">
                                    <strong>storage
                                        vendor {i}</strong>: {rawData["in_storage"][i][sliderValue]} / {rawData["max_storage"]}
                                </Typography>
                            </CardContent>
                        </Card>
                    </>
                ...
                {activeVendor !== -1 && <Card>
                    <CardContent>
                        <Typography variant="h5">
                            <strong>total profit
                                vendor {activeVendor}</strong>: {Number(rawData["profits"][activeVendor].reduce((pv, cv) => pv + cv, 0)).toFixed(2)}
                        </Typography>
                    </CardContent>
                </Card>}
                {activeVendor === -1 && Array.from(Array(rawData["vendors"]).keys()).map(i => {
                    return <>
                        <Card>
                            <CardContent>
                                <Typography variant="h5">
                                    <strong>total profit
                                        vendor {i}</strong>: {Number(rawData["profits"][i].reduce((pv, cv) => pv + cv, 0)).toFixed(2)}
                                </Typography>
                            </CardContent>
                        </Card>
                    </>
    ...
    if (vendor !== -1)
        return {
            ...
            datasets: [
                {
                    label: 'Price New',
                    data: data["price_new"][vendor].slice(lowerBound, upperBound),
                    stepped: true,
                    borderColor: 'red',
                },
                {
                    label: 'Price Refurbished',
                    data: data["price_refurbished"][vendor].slice(lowerBound, upperBound),
                    stepped: true,
                    borderColor: 'green'
                },
                {
                    label: 'Price Rebuy',
                    data: data["price_rebuy"][vendor].slice(lowerBound, upperBound),
                    stepped: true,
                    borderColor: 'blue'
                }
            ]
        ...
        datasets: [
            {
                ...
                data: data["price_new"][0].slice(lowerBound, upperBound),
                stepped: true,
                borderColor: 'red',
            },
            {
                label: 'Price Refurbished from vendor 0',
                data: data["price_refurbished"][0].slice(lowerBound, upperBound),
                stepped: true,
                borderColor: 'green'
            },
            {
                label: 'Price Rebuy from vendor 0',
                data: data["price_rebuy"][0].slice(lowerBound, upperBound),
                stepped: true,
                borderColor: 'blue'
            },
            {
                label: 'Price New from vendor 1',
                data: data["price_new"][1].slice(lowerBound, upperBound),
                stepped: true,
                borderColor: 'orangered',
            },
            {
                label: 'Price Refurbished from vendor 1',
                data: data["price_refurbished"][1].slice(lowerBound, upperBound),
                stepped: true,
                borderColor: 'lightgreen'
            },
            {
                label: 'Price Rebuy  from vendor 1',
                data: data["price_rebuy"][1].slice(lowerBound, upperBound),
                stepped: true,
...
export const getCustomerBehaviour = (slidingPos, episodeLength, data, vendor) => {
